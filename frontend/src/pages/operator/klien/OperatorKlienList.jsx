...
export default function OperatorKlienList() {
  ...
  // 🚀 Ambil data klien (otomatis hanya untuk kecamatan operator via backend)
  ...
  // 🧭 Ambil daftar LKS di kecamatan operator
  const fetchLKS = async () => {
    try {
      const res = await api.get("/lks");
      setDaftarLKS(res.data?.data || []);
    } catch (error) {
      console.error("Gagal memuat daftar LKS:", error);
    }
  };
    ...
    <OperatorLayout>
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">Data Klien Wilayah Anda</h1>
          <button
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          >
            <option value="">Semua LKS di Kecamatan</option>
            {daftarLKS.map((lks) => (
