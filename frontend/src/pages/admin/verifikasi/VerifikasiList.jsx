...
export default function VerifikasiList() {
  ...
  if (!data.data || data.data.length === 0) {
    return <div className="p-4 text-gray-500">Tidak ada data verifikasi.</div>;
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <h1 className="text-xl font-semibold">Verifikasi (Admin)</h1>
        <select
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="p-2 text-left">Petugas</th>
          ...
          <tbody>
            {data.data.map((v) => (
              <tr key={v.id} className="border-t">
                ...
                <td className="p-2">{v.petugas?.name}</td>
                <td className="p-2">
                ...
                <td className="p-2 text-right">
                  <button
                    onClick={() => navigate(`/admin/verifikasi/${v.id}`)}
