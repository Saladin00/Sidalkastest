  ...
  return (
    <div className="space-y-4">
      ...
      {Array.isArray(data.foto_bukti) && data.foto_bukti.length > 0 && (
        <div className="flex gap-3 flex-wrap">
          {data.foto_bukti.map((p, i) => (
            <img key={i} src={`${storage}/${p}`} alt="" className="h-32 rounded border" />
          ))}
        </div>
      )}
      ...
      <div className="border rounded p-3 space-y-2">
        ...
        <textarea className="w-full border p-2 rounded" placeholder="Catatan admin (opsional)"
          value={catatanAdmin} onChange={e=>setCatatanAdmin(e.target.value)} />
        <button onClick={save} className="px-4 py-2 rounded bg-emerald-600 text-white">Simpan Status</button>
      </div>
    </div>
  );
}
