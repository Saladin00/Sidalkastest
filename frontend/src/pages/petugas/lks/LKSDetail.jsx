...
import { ArrowLeftIcon, MapPinIcon, ClipboardDocumentListIcon, CheckCircleIcon } from "@heroicons/react/24/outline";
...
export default function LKSPetugasDetail() {
  ...
  if (!lks) return <div className="p-6 text-center text-gray-500">Memuat data...</div>;
    ...
    <div className="max-w-5xl mx-auto bg-white shadow-sm border border-gray-200 rounded-2xl p-8 md:p-10">
      ...
      <section className="mb-6">
        ...
        <div className="border-l-4 border-blue-200 pl-4 grid grid-cols-1 md:grid-cols-2 gap-y-2 text-sm">
          <p><strong>Nama LKS:</strong> {display(lks.nama)}</p>
          <p><strong>Jenis Layanan:</strong> {display(lks.jenis_layanan)}</p>
          <p><strong>Alamat:</strong> {display(lks.alamat)}</p>
          <p><strong>Kecamatan:</strong> {display(lks.kecamatan)}</p>
          <p><strong>Koordinat:</strong> {display(lks.koordinat)}</p>
        </div>
      </section>
      ...
      {position && (
        <section className="mb-6">
          ...
          <MapContainer center={position} zoom={13} className="h-64 rounded-md border">
      ...
      <div className="flex justify-end mt-8">
        ...
        >
          <CheckCircleIcon className="h-5 w-5" /> Verifikasi Lapangan
        </button>
      </div>
    </div>
  );
